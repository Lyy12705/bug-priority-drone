# Improved Priority Experiment Summary

本次依照改良順序完成五個方向：

1. 針對 P2 錯誤案例加入更多 keyword features。
2. 強化 P1/P2/P3 boundary classifier objective。
3. 繼續調 REP- / BM25F 欄位權重。
4. 加入 XGBoost 作為 improved classifier engine 候選。
5. 評估 transformer 實驗可行性。

## 1. P2 Error-driven Keyword Features

新增的 feature 主要來自 `reports/p2_error_manual_categories_bm25_boundary_best.md` 中的 P2 錯誤型態。
...
## 2. Boundary Classifier Objective

Boundary classifier 保持 P1/P2/P3 refiner 的架構，但新增以下調整：
...
- 加入 collapse penalty，避免只提升 P2 卻讓 P1/P3 崩掉
...
`p2_error_keywords` 的 REP- duplicate-trained weights：
...
`bigram_high` 的 REP- duplicate-trained weights：
...
XGBoost 在這批高維文字特徵上沒有超過 SGD baseline，但 off-by-one accuracy 較高。

## 5. Result Comparison

| Model | Accuracy | Macro F1 | Off-by-one | MAE | P1 recall | P2 recall | P3 recall | P4 recall | P5 recall |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| old best BM25 boundary | 0.7116 | 0.7108 | 0.8955 | 0.4302 | 0.6734 | 0.6515 | 0.8400 | 0.6533 | 0.7387 |
...
- Model：`models/improved_priority_p2_keywords_model.joblib`
- Eval CSV：`reports/improved_priority_p2_keywords_eval.csv`
- Class report：`reports/improved_priority_p2_keywords_class_report.csv`
- Grid report：`reports/improved_priority_p2_keywords_grid_search.md`
- P2 error analysis：`reports/p2_error_analysis_improved_priority_p2_keywords_summary.md`
- P2 manual categories：`reports/p2_error_manual_categories_improved_priority_p2_keywords.md`
...
因此目前建議把 `p2_keywords_sgd` 作為新的最佳模型。

## New P2 Error Analysis
...
Transformer 實驗需要額外安裝套件並下載模型，例如 `sentence-transformers/all-MiniLM-L6-v2`、CodeBERT 或其他 bug-report / software-engineering 語料模型。

建議先不要把 transformer 放進目前主模型，原因是：

- 會增加大量 dependency 與下載模型的成本。
- 訓練與推論時間會明顯增加。
